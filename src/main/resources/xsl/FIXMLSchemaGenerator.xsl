		...
		<xsl:result-document href="{localfn:cleanUrl(concat($targetDir,'/fixml-fields-base',$FileSuffix))}">
			...
			<xso:schema>
				...
				<xsl:for-each select="/fixr:repository/fixr:fields/fixr:field[not(@type='NumInGroup' or ends-with(@name,'SecurityXML') or ends-with(@name,'SecurityXMLLen'))]">
					...
					<xsl:variable name="COMPREF" select="/fixr:repository/fixr:components/fixr:component/fixr:fieldref[@id=$TAGNUM][1]"/>
					...
					<xsl:variable name="MSG" select="/fixr:repository/fixr:messages/fixr:message[@id=9 and fixr:structure/fixr:fieldref/@id=37]"/>
					<!-- If the search is successful in any one of them, then the field is (also) used outside of the session category -->
					<xsl:comment> <xsl:value-of select="@name"/>(<xsl:value-of select="$TAGNUM"/>):<xsl:value-of select="$MSG/@name"/>:<xsl:value-of select="$COMPREF/@id"/>:<xsl:value-of select="$GROUPREF"/>:
					</xsl:comment>
