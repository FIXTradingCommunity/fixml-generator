			...
			<xso:complexType name="{@name}_message_t" final="#all">
				...
				<xso:annotation>
					<xso:documentation xml:lang="en">
						<xsl:value-of select="@name"/> can be found at https://www.fixtrading.org/online-specification/business-area-<xsl:value-of select="$sid"/>/msg<xsl:value-of select="@id"/>.html
					</xso:documentation>
					<xso:appinfo>
						<xsl:element name="fm:Xref" namespace="{$fmNamespace}">
							<xsl:attribute name="Protocol">FIX</xsl:attribute>
